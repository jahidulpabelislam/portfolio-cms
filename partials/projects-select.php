...
<div class="projects-select" ng-show="isLoggedIn">
    ...
    <table class="projects-select__table table table--sticky">
        ...
        <tbody>
            <tr class="table__row" ng-repeat="project in projects">
                ...
                <td class="table__column" data-title="Date">{{ project.date|date:'d MMM yy' }}</td>
                <td class="table__column" data-title="Created At">{{ project.created_at|date:'d MMM yy' }}</td>
                <td class="table__column" data-title="Last Modified">{{ project.updated_at|date:'d MMM yy' }}</td>
                <td class="table__column table__column--center table__column--no-padding">
                    <a class="projects-select__edit-button js-edit-project" href="/project/{{ project.id }}/edit/" title="Link to Edit Project Form Page" tabindex="1" ng-click="selectProject(project)">
                        <span class="screen-reader-text">Edit</span>
                        <i class="fas fa-edit"></i>
                    </a>

                    <button type="button" class="projects-select__delete-button" ng-click="selectProject(project); checkAuthStatus(deleteProject);" tabindex="1">
    ...
    <ul class="pagination" ng-show="pages.length > 1">
        <li class="pagination__item" ng-repeat="page in pages" >
            <a class="pagination__item-link js-projects" href="/projects/{{ page }}/" title="Link to Projects Page {{ page }}" ng-class="{'active': page == currentPage}" data-page="{{ page }}" tabindex="1">{{ page }}</a>
        </li>
    </ul>
</div>
